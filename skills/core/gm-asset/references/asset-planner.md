...
- Visual prompt notes (if any preliminary style decisions exist)
...
- `STYLE.md` — visual prompt anchor, suffix, rules, and avoid list
- `STRUCTURE.md` — architecture and asset hints
- `SCENES.md` — per-scene element lists (drives which references to consult)
- `references/scene_*.png` — per-scene visual targets (generated by `/gm-asset` Step 3 from `visual-target.md`); the visual ground truth `gm-evaluate` will VQA-compare against

## Setup

Read `asset-gen.md` for CLI reference and prompt templates.

## Workflow

### 1. Analyze inputs → identify visual elements

For each scene listed in `SCENES.md`, read its corresponding `references/scene_{name}.png` (if present) — understand the visual composition for that scene: what objects are visible, their proportions, the environment, foreground vs background layers. The composite picture across all scene references tells you which assets to generate and at what scale. If a scene reference is missing, fall back to the SCENES.md textual description and STYLE.md.

Read `STRUCTURE.md` (especially **Asset Hints**) and `PLAN.md` (especially **Assets needed** per task). Cross-reference both with the scene references to build the complete asset list:
...
### 3. Read visual prompt style

Read `STYLE.md`. Use Style Anchor, Prompt Suffix, UI / Asset Rules, and Avoid entries when crafting each asset prompt.
...
- **Backgrounds/panoramas** benefit most from style anchor and prompt suffix language
- **Sprites** may need some style cues but adapted to the subject

Craft each prompt for its specific goal.
...
**Visual style:** <STYLE.md Style Anchor + relevant Prompt Suffix / rules>
