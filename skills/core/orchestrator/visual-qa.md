...
Analyze game screenshots against the visual reference. Typically invoked by a verifier subagent dispatched from the lead agent. Default backend is Gemini 3 Flash; pass `--native` for Claude vision or `--both` for aggregated verdict.
