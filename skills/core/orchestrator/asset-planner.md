...
You receive a worker brief from the lead agent containing:
- `budget_cents` — total asset budget in cents
- Project path — absolute path to the Godot project
- Art direction notes (if any preliminary style decisions exist)

For iteration requests: a specific task description (e.g. "regenerate car model" or "add missing explosion sprite").

Read these project documents before starting:
- `PLAN.md` — game description and task breakdown (especially **Assets needed** per task)
- `STRUCTURE.md` — architecture and asset hints
- `reference.png` — visual target (if already generated)

## Setup

Read `skills/core/orchestrator/asset-gen.md` for CLI reference and prompt templates.
...
When complete, report back to the lead agent:
- Total assets generated (count by type)
- Total cost spent vs budget
- Any assets that failed generation (with reason)
- ASSETS.md location
- List of files created (paths only, not content)
