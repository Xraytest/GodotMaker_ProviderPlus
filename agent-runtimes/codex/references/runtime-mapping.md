...
| `native_image_generation` | Use the active Codex runtime-native image-generation path for `native` and `codex`. Place the generated PNG at the path requested by `/gm-asset`. If unavailable, gate before asset generation. |
